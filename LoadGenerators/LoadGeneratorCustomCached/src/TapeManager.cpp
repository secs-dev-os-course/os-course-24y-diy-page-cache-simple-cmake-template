...
TapeManager::TapeManager(std::string input_tape_name, std::string out_tape_name, size_t ram_bytes)
    ...
    input_fd = files_manager.f_open(input_tape_name.c_str());
    if (input_fd == -1) {
        closeTmp();
        throw std::runtime_error("Error opening input file!");
    }

    output_fd = files_manager.f_open(out_tape_name.c_str());
    if (output_fd == -1) {
        closeTmp();
...
void TapeManager::parseConfig() {
    ...
    while (std::getline(config_file, line)) {
        ...
        if (tokens[0] == "read_delay") read_delay = std::stoi(tokens[1]);
        if (tokens[0] == "write_delay") write_delay = std::stoi(tokens[1]);
        if (tokens[0] == "move_delay") move_delay = std::stoi(tokens[1]);
        if (tokens[0] == "rewind_delay") rewind_delay = std::stoi(tokens[1]);
    }
...
void TapeManager::prepareTapes() {
    for (const auto& entry : fs::directory_iterator(TMP_DIR_NAME)) {
        if (entry.is_regular_file()) {
            ...
            int tmp_tape_fd = files_manager.f_open((fs::path(TMP_DIR_NAME) / filename).c_str());
            ...
            tmp_tapes.push_back(tmp_tape_fd);
            emulateRewindDelay();
        }
    }
}
    ...
    try {
        if (fs::exists(dir_name) && fs::is_directory(dir_name)) {
            for (const auto& entry : fs::directory_iterator(dir_name)) {
                ...
                if (fs::is_directory(path)) clearTmpDir(path);
...
void TapeManager::writeChunk(int32_t chunk[], int32_t elems_in_chunk, size_t tape_number) {
    ...
    std::string tmp_tape_name = "tape_" + std::to_string(tape_number);
    std::filesystem::path tmp_tape_path = fs::path(TMP_DIR_NAME) / tmp_tape_name;

    if (!fs::exists(tmp_tape_path)) {
        std::ofstream tmp_tape(tmp_tape_path);
        if (!tmp_tape.is_open()) {
            throw std::runtime_error("Error creating or opening tmp tape file!");
        }
        tmp_tape.close();
    }

    int tmp_tape_fd = files_manager.f_open(tmp_tape_path.c_str());
    if (tmp_tape_fd == -1) {
        throw std::runtime_error("Error creating or opening tmp tape file!");
    }

    for (size_t j = 0; j < elems_in_chunk; j++) {
        files_manager.f_write(tmp_tape_fd, &chunk[j], sizeof(int32_t));
    }

    emulateSequentialWriteDelay(elems_in_chunk);

    files_manager.f_close(tmp_tape_fd);
}
...
void TapeManager::createTmpTapes() {
    ...
    if (elems_in_chunk) writeChunk(chunk, elems_in_chunk, tmp_tapes_count);
}
...
std::optional<int32_t> TapeManager::readFromTmpTape(int tape_idx) {
    ...
    int32_t number;
    ssize_t bytes_read = files_manager.f_read(tmp_tapes[tape_idx], &number, sizeof(int32_t));
    if (bytes_read > 0) {
...
void TapeManager::writeToOutTape(int32_t value) {
    ...
    files_manager.f_write(output_fd, value_str.c_str(), value_str.size());
}
...
std::optional<int32_t> TapeManager::readFromInputTape() {
    ...
    int32_t value;
    ssize_t bytes_read = files_manager.f_read(input_fd, &value, sizeof(int32_t));
    if (bytes_read > 0) {
...
TapeManager::~TapeManager() {
    closeTmp();
    clearTmpDir(TMP_DIR_NAME);
    files_manager.f_close(input_fd);
    files_manager.f_close(output_fd);
}
