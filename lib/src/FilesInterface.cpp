...
ssize_t FilesManager::f_read(int fd, void* buf, size_t count) {
    ...
    for (int i = first_page_idx; i <= last_page_idx; i++) {
        CacheBlock* cache_block_ptr = page_cache.getCached(fd, i);
        if (cache_block_ptr == nullptr) {
            page_cache.cachePage(fd, i);
            cache_block_ptr = page_cache.getCached(fd, i);
        }
        ...
        std::memcpy(static_cast<char*>(buf) + buff_pos, cache_block_ptr->cached_page.get() + start_pos,
...
ssize_t FilesManager::f_write(int fd, const void* buf, size_t count) {
    ...
    for (int i = first_page_idx; i <= last_page_idx; i++) {
        ...
        std::memcpy(cache_block_ptr->cached_page.get() + start_pos, static_cast<const char*>(buf) + buff_pos,
                    bytes_am * sizeof(char));
        buff_pos += bytes_am;
        bytes_left -= bytes_am;
        curr_offset += bytes_am;
