...
#define CACHE_SIZE 1024  // Количество блоков в кэше
#define BLOCK_SIZE 4096  // Размер блока в байтах
#define MAX_OPEN_FILES 256

// Структура для идентификации файла
typedef struct FileKey {
  dev_t dev;    // Идентификатор устройства
  ino_t inode;  // Идентификатор inode
} FileKey;

// Структура блока кэша
typedef struct CacheBlock {
  FileKey file_key;    // Ключ файла
  off_t block_start;  // Смещение блока в файле
  int frequency;       // Частота использования
  int is_dirty;        // Флаг модификации
...
typedef struct FileMetadata {
  FileKey file_key;  // Ключ файла
  off_t pointer;      // Текущая позиция в файле
...
int cache_count = 0;
FileMetadata file_metadata[MAX_OPEN_FILES] = {0};
...
int get_file_key(int file_descriptor, FileKey* file_key) {
...
int unregister_fd(int file_descriptor) {
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (file_metadata[i].fd == file_descriptor) {
            memset(&file_metadata[i], 0, sizeof(FileMetadata));
            return 0;
        }
    }
    errno = EBADF;
...
int find_block_in_cache(FileKey* file_key, off_t block_offset) {
...
int evict_block() {
    ...
    memset(&cache[least_used_index], 0, sizeof(CacheBlock));
    return least_used_index;
}

// Кэширование блока файла
int load_block_into_cache(FileKey* key, off_t block_offset, size_t* out_bytes_read, bool prefetch) {
    int file_desc = -1;
...
int flush_dirty_blocks(FileKey* key) {
    for (int idx = 0; idx < CACHE_SIZE; idx++) {
...
ssize_t lab2_read(int fd, void* buffer, size_t byte_count) {
    ...
    FileKey file_key = file_meta->file_key;
...
ssize_t lab2_write(int fd, const void* buffer, size_t length) {
    ...
    FileKey file_key = file_info->file_key;
...
int lab2_close(int fd) {
    ...
    for (int idx = 0; idx < CACHE_SIZE; idx++) {
        if (cache[idx].file_key.inode == f_key.inode && cache[idx].file_key.dev == f_key.dev) {
            free(cache[idx].data);
            memset(&cache[idx], 0, sizeof(CacheBlock));
        }
    }
